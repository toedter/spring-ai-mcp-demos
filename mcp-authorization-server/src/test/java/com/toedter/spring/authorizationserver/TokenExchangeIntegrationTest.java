...
/**
 * Verifies the RFC 8693 OAuth 2.0 Token Exchange grant configured for mcp-client-client: exchanging
 * a (simulated) end-user access token together with mcp-client's own client-credentials access
 * token yields a new access token that keeps the original user's {@code sub} claim while adding
...
class TokenExchangeIntegrationTest {
  ...
  void tokenExchangeKeepsUserSubjectAndAddsMcpClientAsActor() throws Exception {
    ...
    String actorToken =
        getAccessToken(tokenUri, "mcp-client-client", "mcp-client-secret", "mcp.tools");
    ...
    String exchangedAccessToken =
        exchangeToken(
            tokenUri, subjectTokenValue, actorToken, "mcp-client-client", "mcp-client-secret");
  ...
  void tokenExchangeChainsMcpServerAsSecondActorOnTopOfMcpClient() {
    ...
    String mcpClientActorToken =
        getAccessToken(tokenUri, "mcp-client-client", "mcp-client-secret", "mcp.tools");
    String mcpClientDelegatedToken =
        exchangeToken(
            tokenUri,
            subjectTokenValue,
            mcpClientActorToken,
    ...
    // mcp-server. -----
    String mcpServerActorToken =
        getAccessToken(tokenUri, "mcp-server-client", "mcp-server-secret", "mcp.tools");
    String mcpServerDelegatedToken =
        exchangeToken(
            tokenUri,
            mcpClientDelegatedToken,
            mcpServerActorToken,
            "mcp-server-client",
            "mcp-server-secret",
            "mcp.tools");
  ...
  private void saveUserAuthorization(String username, String tokenValue) {
    RegisteredClient chatbotClient =
        registeredClientRepository.findByClientId("mcp-chatbot-client");
    ...
    OAuth2AccessToken accessToken =
        new OAuth2AccessToken(
            ...
            issuedAt,
            expiresAt,
            Set.of("openid", "profile", "mcp.tools"));

    Authentication principal =
        new UsernamePasswordAuthenticationToken(
            username, "n/a", List.of(new SimpleGrantedAuthority("SCOPE_mcp.tools")));

    OAuth2Authorization authorization =
        OAuth2Authorization.withRegisteredClient(chatbotClient)
