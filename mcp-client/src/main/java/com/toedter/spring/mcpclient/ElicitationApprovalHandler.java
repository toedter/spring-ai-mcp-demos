...
/**
 * Handles MCP elicitation requests ({@code elicitation/create}) from the configured mcp-server
 * connection. Per the MCP client guidelines, elicitation is for gathering ordinary information on
 * demand and MUST NOT be used as a credential channel — any request that looks like it is asking
 * for a password, API key, or similar secret is automatically declined without ever prompting the
 * user.
 *
 * <p>Only {@code form}-mode requests (a flat JSON Schema of top-level properties) are supported,
 * matching what the mcp-chatbot form renderer understands; {@code url}-mode requests are declined.
 */
@Component
public class ElicitationApprovalHandler {
  ...
  private static final Set<String> CREDENTIAL_MARKERS =
      Set.of("password", "passwd", "secret", "api key", "apikey", "api_key", "token", "credential");
    ...
    return switch (action) {
      case "accept" -> {
        ...
        Map<String, Object> content =
            (Map<String, Object>) decision.getOrDefault("content", Map.of());
  ...
  /**
   * Scans the requested schema's property names, titles, and descriptions for words that indicate
   * the server is trying to use elicitation as a credential channel, which the MCP client
   * guidelines forbid.
   */
  @SuppressWarnings("unchecked")
  private static boolean asksForCredentials(ElicitFormRequest request) {
    ...
    for (Map.Entry<?, ?> entry : properties.entrySet()) {
      ...
      if (entry.getValue() instanceof Map<?, ?> propertySchema) {
        ...
        if (containsCredentialMarker(String.valueOf(title))
            || containsCredentialMarker(String.valueOf(description))) {
          return true;
        }
      }
    }
