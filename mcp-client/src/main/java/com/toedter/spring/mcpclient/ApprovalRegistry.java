...
/**
 * Coordinates every kind of human-in-the-loop decision this client requires
 * before a server-initiated action proceeds: tool-call approval, sampling
 * approval, and elicitation responses. Each pending decision is registered
 * under a generated id; the caller blocks on {@link #await} while the
 * decision arrives asynchronously (from the browser) via one of
 * {@link ChatController}'s decision endpoints, which calls {@link #complete}.
 */
@Component
public class ApprovalRegistry {
    ...
    /**
     * Block until the decision for {@code id} arrives or the timeout elapses,
     * returning {@code onTimeout} in the latter case (fail closed).
     */
