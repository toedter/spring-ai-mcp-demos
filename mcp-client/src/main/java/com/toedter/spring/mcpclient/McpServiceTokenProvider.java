...
/**
 * Fetches (and caches) an OAuth2 client-credentials access token used to authenticate outgoing MCP
 * transport traffic (initialize, listTools, callTool, keep-alive pings, ...) against the
 * mcp-server, which is secured as an OAuth2 resource server requiring the {@code mcp.tools} scope.
 *
 * <p>Uses the same {@code mcp-client-client} client-credentials registration as {@code
 * mcp-authorization-server/get-access-token.http}.
 *
 * <p>Tokens are cached per MCP server connection name so that a token is never reused across
 * servers, even though this demo only configures one.
 */
@Component
public class McpServiceTokenProvider {
  ...
  /**
   * Returns a valid access token scoped to {@code serverName}, fetching (or refreshing) it as
   * needed. Each MCP server connection gets its own cached token so a token is never reused across
   * servers.
   */
