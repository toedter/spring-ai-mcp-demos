...
/**
 * Verifies tokens are cached per MCP server connection name (guideline: never
 * reuse a token across servers), by counting how many times the token
...
class McpServiceTokenProviderTest {
    ...
    void startFakeTokenEndpoint() throws IOException {
        ...
        server.createContext("/oauth2/token", exchange -> {
            ...
            String body = "{\"access_token\":\"token-" + requestCount.get() + "\",\"expires_in\":300}";
        ...
        tokenProvider = new McpServiceTokenProvider(tokenUri, "client-id", "client-secret", "mcp.tools");
    }
