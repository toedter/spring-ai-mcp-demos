...
/**
 * Verifies delegated (exchanged) tokens are cached per
 * {@code (serverName, subjectAccessToken)} rather than globally, so the same
 * user's exchanged token is never reused across MCP server connections.
 */
class TokenExchangeServiceTest {
    ...
    void startFakeTokenEndpoint() throws IOException {
        ...
        server.createContext("/oauth2/token", exchange -> {
            ...
            String body = "{\"access_token\":\"exchanged-" + requestCount.get() + "\",\"expires_in\":300}";
    ...
    void cachesExchangedTokenPerServerNameForTheSameSubjectToken() {
        ...
        String exchangedForServerA1 = tokenExchangeService.exchangeUserToken("server-a", userToken, actorToken);
        String exchangedForServerA2 = tokenExchangeService.exchangeUserToken("server-a", userToken, actorToken);
        String exchangedForServerB = tokenExchangeService.exchangeUserToken("server-b", userToken, actorToken);
