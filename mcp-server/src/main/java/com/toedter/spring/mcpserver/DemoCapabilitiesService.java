...
/**
 * Demo tools that exercise mcp-client's sampling and elicitation support end to end, so those MCP
 * client capabilities can be verified through the real chatbot rather than only with unit tests.
 */
@Service
public class DemoCapabilitiesService {

  @McpTool(
      name = "write_haiku_about_weather",
      description =
          "Writes a short haiku about the weather in a city, generated via the client's LLM "
              + "(MCP sampling).",
  ...
  public String writeHaikuAboutWeather(
    ...
    CreateMessageRequest request =
        CreateMessageRequest.builder(List.of(message), 200)
  ...
  @McpTool(
      name = "get_preferred_temperature_unit",
  ...
  public String getPreferredTemperatureUnit(McpSyncServerExchange exchange) {
    ...
    ElicitFormRequest elicitRequest =
        ElicitFormRequest.builder("What temperature unit do you prefer?", requestedSchema).build();
    ...
    return unit == null
        ? "The user did not provide a preferred temperature unit."
        : String.valueOf(unit);
  }
}
